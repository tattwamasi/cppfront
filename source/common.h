...
struct source_line {
  ...
  enum class category : u8 {
    ...
    cpp2,
    rawstring,
    preprocessor_was_emitted
  };
  category cat;

  bool all_tokens_are_densely_spaced =
      true; // to be overridden in lexing if they're not

  source_line(std::string_view t = {}, category c = category::empty)
      : text{t}, cat{c} {}

  auto indent() const -> int {
    return unchecked_narrow<int>(
        std::find_if_not(text.begin(), text.end(), &isspace) - text.begin());
  }
  ...
  auto prefix() const -> std::string {
    ...
    case category::empty:
      return "/*   */ ";
      break;
    case category::preprocessor:
      return "/* # */ ";
      break;
    case category::comment:
      return "/* / */ ";
      break;
    case category::module_directive:
      return "/* m#*/ ";
      break;
    case category::module_declaration:
      return "/* m */ ";
      break;
    ...
    case category::cpp2:
      return "/* 2 */ ";
      break;
    case category::rawstring:
      return "/* R */ ";
      break;
    default:
      assert(false && "ICE: illegal category");
      abort();
    }
  }
};
...
using colno_t =
    int32_t; // not int16_t... encountered >80,000 char line during testing
...
struct source_position {
  ...
  source_position(lineno_t l = 1, colno_t c = 1) : lineno{l}, colno{c} {}
...
struct string_parts {
  ...
  string_parts(const std::string &beginseq, const std::string &endseq,
               adds_sequences strateg)
      : begin_seq{beginseq}, end_seq{endseq}, strategy{strateg} {
  ...
  void add_string(const std::string &text) {
    parts.push_back(raw_string{text});
  }
  void add_string(const std::string_view &text) {
    parts.push_back(raw_string{std::string(text)});
  }
  ...
  auto generate() const -> std::string {
    ...
    auto result = std::visit(begin_visit{begin_seq, strategy}, parts.front());
...
private:
  ...
  struct end_visit {
    ...
    auto operator()(const cpp_code &) const -> std::string { return {}; }
  };
  ...
  struct generator_visit {
    ...
    auto operator()(const raw_string &, const cpp_code &part) const
        -> std::string {
      return end_seq + " + " + part.text;
    }
    auto operator()(const cpp_code &, const raw_string &part) const
        -> std::string {
      return " + " + begin_seq + part.text;
    }
    auto operator()(const raw_string &, const raw_string &part) const
        -> std::string {
      return part.text;
    }
    auto operator()(const cpp_code &, const cpp_code &part) const
        -> std::string {
      return " + " + part.text;
    }
  };
...
// G     one of '0' '1'
// G
auto is_binary_digit(char c) -> bool { return c == '0' || c == '1'; }

// G digit: one of
// G     binary-digit
// G     one of '2' '3' '4' '5' '6' '7' '8' '9'
// G
auto is_digit(char c) -> bool { return isdigit(c); }

// G hexadecimal-digit:
// G     digit
// G     one of 'A' 'B' 'C' 'D' 'E' 'F'
// G
auto is_hexadecimal_digit(char c) -> bool { return isxdigit(c); }
...
// G     _
// G
auto is_nondigit(char c) -> bool { return isalpha(c) || c == '_'; }

// G identifier-start:
// G     nondigit
// G
auto is_identifier_start(char c) -> bool { return is_nondigit(c); }
...
// G identifier:
// G     '__identifier__' keyword    [Note: without whitespace before the
// keyword] G     identifier-start G     identifier identifier-continue G
// 'operator' operator
// G
auto starts_with_identifier(std::string_view s) -> int {
  if (is_identifier_start(s[0])) {
    auto j = 1;
    while (j < std::ssize(s) && is_identifier_continue(s[j])) {
      ++j;
    }
    return j;
  }
...
//
auto is_separator_or(auto pred, char c) -> bool { return c == '\'' || pred(c); }

auto peek(std::string_view line, int i, int num) {
  return (i + num < std::ssize(line) && i + num >= 0) ? line[i + num] : '\0';
}
...
auto is_encoding_prefix_and(std::string_view line, int i, char next) {
  ...
  if (line[i] == next) {
    ...
    else if (peek1 == '8' && peek2 == next) {
      return 3;
    } // u8"
    else if (peek1 == 'R' && peek2 == next) {
      return 3;
    } // uR"
    else if (peek1 == '8' && peek2 == 'R' && peek3 == next) {
      return 4;
    } // u8R"
  } else if (line[i] == 'U') {
    if (peek1 == next) {
...
//  Explicit cast
//
template <typename T> auto _as(auto x) -> T { return T(x); }
...
template <std::size_t N> struct String {
  constexpr String(const char (&str)[N]) { std::copy_n(str, N, value); }
...
auto is_empty_or_a_decimal_number(std::string_view s) -> bool {
...
auto starts_with(std::string const &s, std::string_view sv) -> bool {
...
auto contains(std::string const &s, auto const &value) -> bool {
...
class cmdline_processor {
  ...
  struct flag {
    ...
    flag(int g, std::string_view n, std::string_view d, callback0 h0,
         callback1 h1, std::string_view s, bool o)
        : group{g}, name{n}, description{d}, handler0{h0}, handler1{h1},
          synonym{s}, opt_out{o} {}
  ...
  static auto print(std::string_view s, int width = 0) -> void {
    if (width > 0) {
...
public:
  auto flags_starting_with(std::string_view s, bool indicate_short_name = true)
  ...
  auto process_flags() -> void {
    ...
    for (auto flag1 = flags.begin(); flag1 != flags.end(); ++flag1) {
      for (auto flag2 = flag1 + 1; flag2 != flags.end(); ++flag2) {
        int i = 0;
        while (i < std::ssize(flag1->name) && i < std::ssize(flag2->name) &&
    ...
    for (auto arg = args.begin(); arg != args.end(); ++arg) {
      ...
      for (auto &flag : flags) {
        auto length_to_match =
            std::max(flag.unique_prefix, _as<int>(arg->text.length()) - 1);
        if (flag.opt_out && arg->text.ends_with("-")) {
          length_to_match =
              std::max(flag.unique_prefix, _as<int>(arg->text.length()) - 2);
        }
        ...
        if (arg->text.starts_with("-" + flag.name.substr(0, length_to_match)) ||
          ...
          //  Else
            ...
            //  Else this is a switch that takes the next arg as its value, so
            //  pass that
            else {
              if (arg + 1 == args.end()) {
                print("Missing argument to option " + arg->text +
                      " (try -help)\n");
                help_requested = true;
                break;
              }
              arg->pos = processed;
              ++arg; // move to next argument, which is the argument to this
                     // switch
  ...
  auto add_flag(int group, std::string_view name, std::string_view description,
                callback0 handler0, callback1 handler1,
                std::string_view synonym, bool opt_out) -> void {
  ...
  struct register_flag {
    register_flag(int group, std::string_view name,
                  std::string_view description, callback0 handler0,
                  callback1 handler1 = {}, std::string_view synonym = {},
                  bool opt_out = false);
  };
  ...
  auto help_was_requested() -> bool { return help_requested; }

  auto arguments() -> std::vector<arg> & { return args; }
...
cmdline_processor::register_flag::register_flag(
    int group, std::string_view name, std::string_view description,
    callback0 handler0, callback1 handler1, std::string_view synonym,
    bool opt_out) {
...
static cmdline_processor::register_flag cmd_help(
    0, "help", "Print help", [] { cmdline.print_help(); }, nullptr, "?");

static cmdline_processor::register_flag
    cmd_version(0, "version", "Print version information",
                [] { cmdline.print_version(); });

static cmdline_processor::register_flag
    cmd_gen_build(0, "_gen_build", "Generate build information",
                  [] { cmdline.gen_build(); });

static auto flag_verbose = false;
static cmdline_processor::register_flag
    cmd_verbose(9, "verbose", "Print verbose output and statistics",
                [] { flag_verbose = true; });

static auto flag_internal_debug = false;
static cmdline_processor::register_flag
    cmd_internal_debug(0, "_debug",
...
struct error_entry {
  ...
  error_entry(source_position w, std::string_view m, bool i = false,
              bool f = false)
      : where{w}, msg{m}, internal{i}, fallback{f} {}

  auto operator==(error_entry const &that) -> bool {
    return where == that.where && msg == that.msg;
  }
...
public:
  stable_vector(std::initializer_list<T> init = {}) { add_segment(init); }

  auto empty() const -> bool { return data.size() == 1 && data.back().empty(); }
  ...
  auto back() -> T & { return data.back().back(); }
  ...
  auto pop_back() -> void {
    ...
    if (data.back().empty() && data.size() > 1) {
      data.pop_back();
    }
    data.back().pop_back();
  }
  ...
  public:
    ...
    auto operator+=(size_t off) -> iterator {
      if (pos + off < v->size()) {
        pos += off;
      } else {
        pos = v->size();
  ...
  public:
    ...
    const_iterator(stable_vector const *v_ = nullptr, size_t pos_ = 0)
        : v{v_}, pos{pos_} {}
    ...
    auto operator+=(size_t off) -> const_iterator {
      if (pos + off < v->size()) {
        pos += off;
      } else {
        pos = v->size();
    ...
    auto operator-(const_iterator const &that) -> ptrdiff_t {
      return pos - that.pos;
    }
    auto operator<=>(const_iterator const &) const
...
template <typename T>
auto operator+(size_t off, typename stable_vector<T>::iterator const &it) ->
    typename stable_vector<T>::iterator {
  auto i = it;
  i += off;
  return i;
}
...
template <typename T>
auto operator+(size_t off, typename stable_vector<T>::const_iterator const &it)
    -> typename stable_vector<T>::const_iterator {
  auto i = it;
  i += off;
  return i;
}
...
namespace std {
template <typename T> auto size(cpp2::stable_vector<T> const &v) -> ptrdiff_t {
  return v.size();
}
template <typename T> auto ssize(cpp2::stable_vector<T> const &v) -> ptrdiff_t {
  return v.ssize();
}
} // namespace std
namespace cpp2 {
      ...
      if (e.ptr) {
        std::cout << "  " << std::setw(6)
                  << ((std::abs(e.delta) < 1000000) ? std::to_string(e.delta)
                                                    : "-----")
                  << " " << std::setw(8)
                  << ((std::abs(e.delta) < 1000000)
                          ? std::to_string(e.cumulative)
                          : "-------")
...
public:
  struct guard {
    guard(std::string_view /*name*/, std::string_view /*file*/, int /*line*/,
          char * /*p*/) {
      ...
      //         deepest = entries;
      //     }
      //     if (largest.empty() || largest.back().cumulative <
      //     entries.back().cumulative) {
      //         largest = entries;
      //     }
      // }
    }
...
#define STACKINSTR                                                             \
  stackinstr::guard _s_guard{__func__, __FILE__, __LINE__,                     \
                             reinterpret_cast<char *>(&_s_guard)};
...
static std::unordered_map<std::string_view, timer>
    timers; // global named timers
...
#define CPP2_SCOPE_TIMER(name)                                                 \
  auto CPP2_UNIQUE_NAME(timer, __LINE__) = scope_timer(name)
#else
#define CPP2_SCOPE_TIMER(name)
#endif
