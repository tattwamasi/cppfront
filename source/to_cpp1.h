...
//  Exceptions. See https://github.com/hsutter/cppfront/blob/main/LICENSE for
//  license information.
...
auto pad(int padding) -> std::string_view {
  ...
  return {indent_str.c_str(),
          _as<size_t>(std::min(padding, _as<int>(std::ssize(indent_str))))};
}

auto multi_return_type_name(declaration_node const &n) -> std::string {
...
static cmdline_processor::register_flag
    cmd_emit_cppfront_info(8, "emit-cppfront-info",
                           "Emit cppfront version/build in Cpp1 file",
                           [] { flag_emit_cppfront_info = true; });

static auto flag_clean_cpp1 = false;
static cmdline_processor::register_flag
    cmd_clean_cpp1(8, "clean-cpp1", "Emit clean Cpp1 without #line directives",
                   [] { flag_clean_cpp1 = true; });

static auto flag_line_paths = false;
static cmdline_processor::register_flag
    cmd_line_paths(8, "line-paths", "Emit absolute paths in #line directives",
                   [] { flag_line_paths = true; });

static auto flag_import_cpp2_util = false;
static cmdline_processor::register_flag
    cmd_import_cpp2_util(9, "import-cpp2-util",
                         "Imports all cpp2:: via 'import cpp2.util;' - "
                         "'#include's the preprocessor parts",
...
static cmdline_processor::register_flag
    cmd_include_std(0, "include-std", "#include all std:: headers",
                    [] { flag_include_std = true; });

static auto flag_cpp2_only = false;
static cmdline_processor::register_flag
    cmd_cpp2_only(0, "pure-cpp2",
...
private:
  ...
  auto inc_phase() -> void {
    ...
    case phase1_type_defs_func_decls:
      ...
      break;
    default:
      assert(false && "ICE: invalid lowering phase");
    }
    curr_pos = {};
  ...
  std::vector<std::string *>
      emit_string_targets; // option to emit to string instead of out file
  std::vector<std::vector<text_with_pos> *>
      emit_text_chunks_targets; // similar for vector<text_pos>
  ...
  //  Print text
  //
  auto print(std::string_view s, source_position pos = source_position{},
             bool track_curr_pos = true, bool is_known_empty = false) -> void {
    //  Take ownership of (and reset) just_printed_line_directive value
    auto line_directive_already_done =
    ...
    if (!emit_target_stack.empty()) {
      ...
      //  If capturing to a vector of chunks, emit to that
        ...
        emit_text_chunks_targets.back()->insert(
            emit_text_chunks_targets.back()->begin(),
    ...
    if ((s == "\n" || is_known_empty) && curr_pos.colno <= 1) {
    ...
    *out << s;
    ...
    if (track_curr_pos) {
      ...
      auto newline_pos =
          std::size_t(0); // the current newline we found in the string
      ...
      if (last_newline != s.npos) {
        //  If we found a newline, it's the distance from the last newline to
        //  EOL
  ...
  //
  auto print_comment(comment const &c) -> void {
    ...
    //  For a stream comment, pad out to its column (if we haven't passed it
    //  already) and emit it there
    else {
  ...
  auto flush_comments(source_position pos,
                      bool print_remaining_comments = false) -> void {
    if (!pcomments) {
      return;
    }
    ...
    //
    while (print_remaining_comments ? (next_comment < std::ssize(comments))
      ...
      if (next_comment < std::ssize(comments) &&
        ...
        assert(pparser);
        if ((phase == phase1_type_defs_func_decls &&
             !pparser->is_within_function_body(
                 comments[next_comment].start.lineno)) ||
            (phase == phase2_func_defs &&
             pparser->is_within_function_body(
                 comments[next_comment].start.lineno))) {
  ...
  auto align_to(source_position pos) -> void {
    ...
    assert(psource && 0 <= curr_pos.lineno &&
    ...
    if (curr_pos.lineno == pos.lineno &&
      ...
      if (psource->get_lines()[curr_pos.lineno].all_tokens_are_densely_spaced) {
        ...
        //  For later tokens, don't try to add padding
          ...
          if (last_printed_char == ';' && on_same_line) {
            print(" ");
          }
        }
      }
...
public:
  ...
  auto finalize_phase(bool print_remaining_comments = false) {
    if (is_open() && psource && psource->has_cpp2()) {
  ...
  //  Open
  //
  auto open(std::string const &cpp2_filename_,
            std::string const &cpp1_filename_,
            std::vector<comment> const &comments, cpp2::source const &source,
            cpp2::parser const &parser) -> void {
    cpp2_filename =
        (flag_line_paths)
            ? std::filesystem::absolute(std::filesystem::path(cpp2_filename_))
                  .string()
            : cpp2_filename_;
  ...
  auto print_cpp1(std::string_view s, lineno_t line) -> void {
    assert(is_open() && line >= 0 &&
           "ICE: printer must be open before printing, and line number must "
           "not be negative (Cpp1 code is never generated)");
  ...
  //
  auto reset_line_to(lineno_t line, bool force = false) -> void {
    //  Always start a Cpp2 section on its own new line
    ensure_at_start_of_new_line();

    //  If we are out of sync with the current logical line number,
    //  emit a #line directive to re-sync
    if (force || curr_pos.lineno != line) {
      print_line_directive(line);
      curr_pos.lineno = line;
    }
  ...
  //
  auto print_cpp2(std::string_view s, source_position pos,
                  bool leave_newlines_alone = false, bool is_known_empty = false
    ...
    if (auto newline_pos = s.find('\n');
        !leave_newlines_alone && s.length() > 1 && newline_pos != s.npos) {
    ...
    if (emit_target_stack.empty()) {
      ...
      //  We may want to adjust the position based on (1) a position preemption
      //  request or else (2) to repeat a similar adjustment we discovered on
      //  the previous line
      ...
      //      and only on the same line. In the future, we might have more use
      //      cases.
      ...
      //      non-comment non-whitespace text on the new line
        ...
        //  If the last line had a request for this colno, remember its actual
        //  offset
        constexpr int sentinel = -100;
        auto last_line_offset = sentinel;
        for (auto i = 0; i < std::ssize(prev_line_info.requests) &&
      ...
      //  If we're changing lines, start accumulating this new line's
      //  request/actual adjustment info
      ...
      prev_line_info.requests.push_back(req_act_info(
          pos.colno /*requested*/, curr_pos.colno /*actual*/ - pos.colno));
    }
  ...
  auto next_phase() -> void { inc_phase(); }
...
class cppfront {
  ...
  using source_order_name_lookup_res = std::optional<
      std::variant<declaration_node const *, active_using_declaration>>;
  ...
  //  their .epilog, by reference for performance while still having lifetime
  //  safety
  struct function_info {
    ...
    function_info(declaration_node const *decl_,
                  function_type_node const *func_,
                  declaration_node::declared_value_set_funcs
                      declared_value_set_functions_)
        : decl{decl_}, func{func_}, declared_value_set_functions{std::move(
                                        declared_value_set_functions_)} {}
  };
  ...
  public:
    auto push(declaration_node const *decl, function_type_node const *func,
              declaration_node::declared_value_set_funcs const &thats) {
      list.emplace_back(decl, func, thats);
    }

    auto pop() { list.pop_back(); }
  ...
  struct function_return {
    ...
    function_return(parameter_declaration_list_node *param_list_,
                    passing_style pass_ = passing_style::invalid,
                    bool is_deduced_ = false)
        : param_list{param_list_}, pass{pass_}, is_deduced{is_deduced_} {}
  };
  std::vector<function_return> function_returns;
  parameter_declaration_list_node single_anon;
  //  special value - hack for now to note single-anon-return type kind in this
  //  function_returns working list
  ...
  auto push_need_expression_list_parens(bool b) -> void {
    need_expression_list_parens.push_back(b);
  }
  auto pop_need_expression_list_parens() -> void {
    assert(std::ssize(need_expression_list_parens) > 1);
    need_expression_list_parens.pop_back();
  }
  auto should_add_expression_list_parens() -> bool {
    assert(!need_expression_list_parens.empty());
    return need_expression_list_parens.back();
  }
  auto consumed_expression_list_parens() -> void {
    if (std::ssize(need_expression_list_parens) > 1)
...
public:
  ...
  cppfront(std::string const &filename)
      : sourcefile{filename}, source{errors}, tokens{errors},
        parser{errors, includes}, sema{errors} {
    ...
    if (!sourcefile.ends_with(".cpp2") && !sourcefile.ends_with(".h2") &&
      ...
      errors.emplace_back(source_position(-1, -1),
                          "source filename must end with .cpp2 or .h2: " +
                              sourcefile);
    }
  ...
  auto lower_to_cpp1() -> lower_to_cpp1_ret {
    ...
    printer.open(sourcefile, cpp1_filename, tokens.get_comments(), source,
                 parser);
    if (!printer.is_open()) {
      errors.emplace_back(source_position{},
    ...
    auto print_cpp2util = [&]() {
      if (!source.is_module_cpp2_util() &&
          (!tokens.get_map().empty() || flag_import_std || flag_include_std)) {
    ...
    auto emit_cpp1_line = [&](auto const &line) {
      if (source.has_cpp2() && line.cat == source_line::category::empty) {
        ++ret.cpp2_lines;
      } else {
        ++ret.cpp1_lines;
      }
        ...
        if (line.cat == source_line::category::preprocessor) {
          if (!line.text.ends_with(".h2\"")) {
            errors.emplace_back(
                ...
                "pure-cpp2 switch disables the preprocessor, including "
                "#include (except of .h2 files) - use import instead (note: "
                "'import std;' is implicit in -pure-cpp2)");
            return false;
          }
        } else {
          errors.emplace_back(source_position(curr_lineno, 1),
    ...
    if (source.has_cpp2()) {
      for (lineno_t curr_lineno = 0; auto &line : source.get_lines()) {
        if (line.cat == source_line::category::preprocessor &&
            contains(line.text, "#include") && !line.text.ends_with(".h2\"")) {
          printer.print_cpp1(line.text, curr_lineno);
          line.cat = source_line::category::preprocessor_was_emitted;
        } else if (line.cat != source_line::category::empty &&
                   line.cat != source_line::category::comment) {
          break;
        }
        ++curr_lineno;
    ...
    if (source.has_cpp2() || flag_import_std || flag_include_std) {
...
#include "build.info"
    ...
    if (flag_import_cpp2_util && source.has_cpp2() &&
    ...
    if (source.has_cpp2() && !flag_clean_cpp1) {
      printer.print_extra(
    ...
    if (source.has_cpp2() && !flag_clean_cpp1) {
      printer.print_extra("\n//=== Cpp2 type definitions and function "
                          "declarations ===========================\n\n");
      printer.reset_line_to(1, true);
    }
    ...
    if (!source.has_cpp2() && source.is_module()) {
      curr_lineno -=
          unchecked_narrow<int>(std::ssize(source.get_module_lines()) - 1);
    }

    assert(printer.get_phase() ==
           positional_printer::phase1_type_defs_func_decls);
      ...
      if (curr_lineno != 0) {
        ...
        //  If it's a Cpp2 line...
          ...
          if (map_iter != tokens.get_map().cend() &&
            ...
            //  We should be here only when we're at exactly the first line of a
            //  Cpp2 section
            assert(map_iter->first == curr_lineno);
            assert(!map_iter->second.empty());

            //  Get the parse tree for this section and emit each forward
            //  declaration
            auto decls =
                parser.get_parse_tree_declarations_in_range(map_iter->second);
            for (auto &decl : decls) {
              assert(decl);
              emit(*decl);
            }
    ...
    if (cpp1_filename.back() == 'h' && flag_cpp2_only) {
      printer.print_extra("\n#endif\n");

      printer.reopen();
      if (!printer.is_open()) {
        errors.emplace_back(source_position{},
                            "could not open second output file " +
                                cpp1_filename);
        return {};
      }
      ...
      printer.print_extra(
          "\n#error This file is part of a '.h2' header compiled to be "
          "consumed from another -pure-cpp2 file. To use this file, write "
          "'#include \"" +
          cpp1_filename +
          "2\"' in a '.h2' or '.cpp2' file compiled with -pure-cpp2.");
      printer.print_extra("\n#endif\n");
    ...
    assert((!errors.empty() || tokens.num_unprinted_comments() == 0) &&
  ...
  //
  template <int I> auto try_emit(auto &v, auto &&...more) -> void {
    if (v.index() == I) {
  ...
  //-----------------------------------------------------------------------
  //
  auto emit(token const &n, bool is_qualified = false, source_position pos = {})
      -> void {
    STACKINSTR
    ...
    if (!is_qualified &&
        (n.type() == lexeme::Cpp2FixedType || n == "finally" ||
         n == "cpp1_ref" || n == "cpp1_rvalue_ref" || n == "unchecked_narrow" ||
         n == "unchecked_cast" || n == "unchecked_cmp_less" ||
         n == "unchecked_cmp_less_eq" || n == "unchecked_cmp_greater" ||
         n == "unchecked_cmp_greater_eq" || n == "unchecked_div" ||
         n == "unchecked_dereference" || n == "unchecked_subscript")) {
    ...
    else if (n == "and" || n == "and_eq" || n == "bitand" || n == "bitor" ||
             n == "compl" || n == "not" || n == "not_eq" || n == "or" ||
             n == "or_eq" || n == "xor" || n == "xor_eq" || n == "new" ||
             n == "class" || n == "struct" || n == "enum" || n == "union" ||
             n == "default") {
      printer.print_cpp2("cpp2_" + n.to_string(), pos);
    } else if (!is_qualified && n == "type_of") {
  ...
  //-----------------------------------------------------------------------
  //
  auto emit(unqualified_id_node const &n, int synthesized_multi_return_size = 0,
            bool is_local_name = true, bool is_qualified = false,
            bool is_class_member_access = false) -> void {
    STACKINSTR
    assert(n.identifier);
    ...
    bool add_forward = last_use && last_use->is_forward &&
                       last_use->safe_to_move &&
                       !in_non_rvalue_context.back() && !is_class_member_access;
    ...
    const bool add_std_move =
        ...
        (synthesized_multi_return_size == 1 && decl && !decl->initializer);

    //  Otherwise we'll use cpp2::move
    bool add_move =
        !add_forward && !n.identifier->as_string_view().starts_with("guard") &&
        (!last_use || last_use->safe_to_move) &&
        (add_std_move || (last_use && !suppress_move_from_last_use)) &&
        !in_non_rvalue_context.back() && !is_class_member_access;
    ...
    const bool add_this = add_move && decl && decl->identifier &&
                          *decl->identifier == "this" &&
                          *n.identifier != "this";

    if (add_move && in_single_unqualified_id_return.size() > 0 &&
        in_single_unqualified_id_return.back() &&
        (!decl || decl->initializer)) {
      add_move = false;
    }

    if (emitting_move_that_function && *n.identifier == "that") {
      add_move = true;
    }
    ...
    if (add_forward) {
      printer.print_cpp2("CPP2_FORWARD(",
                         {n.position().lineno, n.position().colno - 8});
    }
    ...
    assert(n.identifier);
    emit(*n.identifier,
         is_qualified); // inform the identifier if we know this is qualified
    ...
    if (!in_definite_init && !in_parameter_list) {
      if (is_local_name && !(*n.identifier == "this") &&
          !(*n.identifier == "that") && decl &&
          (synthesized_multi_return_size != 0
           //  note pointer equality: if we're not in the actual declaration of
           //  n.identifier
          ...
          &&
          (!decl->parameter ||
           (decl->parameter && decl->parameter->pass == passing_style::out))) {
    ...
    if (!add_this && (add_move || add_std_move || add_forward)) {
  ...
  //-----------------------------------------------------------------------
  //
  auto emit(qualified_id_node const &n, bool include_unqualified_id = true)
      -> void {
    STACKINSTR
    if (!sema.check(n)) {
      return;
    ...
    for (auto const &id :
         std::span{n.ids}.first(n.ids.size() - !include_unqualified_id)) {
      if (id.scope_op) {
        emit(*id.scope_op);
      }
      emit(*id.id, 0, true,
           true); // inform the unqualified-id that it's qualified
    }
  ...
  //-----------------------------------------------------------------------
  //
  auto emit(type_id_node const &n, source_position pos = {},
    ...
    if (n.is_function_typeid()) {
      //  If identifier is nonempty, we're doing a local variable with a
      //  (pointer to) function typeid, so stick in the pointers here for
      //  inside-out Cpp1 declarations
      if (!identifier.empty()) {
      ...
      emit(*std::get<type_id_node::function>(n.id), false, false, {}, false,
           identifier);
      return;
    }
  ...
  //-----------------------------------------------------------------------
  //
  auto emit(id_expression_node const &n, bool is_local_name = true,
            bool is_class_member_access = false) -> void {
    STACKINSTR
    try_emit<id_expression_node::qualified>(n.id);
    try_emit<id_expression_node::unqualified>(n.id, 0, is_local_name, false,
                                              is_class_member_access);
  }

  auto emit_prolog_mem_inits(function_prolog const &prolog, colno_t indent)
      -> void {
    STACKINSTR
  ...
  auto emit_prolog_statements(function_prolog const &prolog, colno_t indent)
      -> void {
    STACKINSTR
  ...
  auto emit_epilog_statements(std::vector<std::string> const &epilog,
                              colno_t indent) -> void {
    STACKINSTR
  ...
  //-----------------------------------------------------------------------
  //
  auto emit(compound_statement_node const &n,
            function_prolog const &function_prolog = {},
            std::vector<std::string> const &function_epilog = {}) -> void {
    STACKINSTR
  ...
  auto emit(inspect_expression_node const &n, bool is_expression) -> void {
    ...
    printer.print_cpp2("{ " + constexpr_qualifier + "auto&& _expr = ",
                       n.position());
    ...
    for (auto first = true; auto &&alt : n.alternatives) {
      ...
      if (*alt->is_as_keyword == "is") {
        ...
        //  If this is an inspect-expression, we'll have to wrap each
        //  alternative in an 'if constexpr' so that its type is ignored for
        //  mismatches with the inspect-expression's type
        auto return_prefix = std::string{};
        auto return_suffix = std::string{
            ";"}; // use this to tack the ; back on in the alternative body
        if (is_expression) {
          return_prefix =
              "{ if constexpr( requires{" + statement +
              ";} ) if constexpr( std::is_convertible_v<CPP2_TYPEOF((" +
              statement + "))," + result_type + "> ) return ";
          return_suffix += " }";
        }
        ...
        if (id == "auto") {
          ...
          if (alt->type_id) {
            printer.print_cpp2("(cpp2::impl::is<" + id + ">(_expr)) ",
                               alt->position());
          } else {
            assert(alt->value);
            printer.print_cpp2("(cpp2::impl::is(_expr, " + id + ")) ",
                               alt->position());
          }
        ...
        if (is_expression && id != "auto") {
          ...
          printer.print_cpp2("; else return " + result_type + "{}",
                             alt->position());
          printer.print_cpp2("; else return " + result_type + "{}",
                             alt->position());
        }
        ...
        errors.emplace_back(
            alt->position(),
            "(temporary alpha limitation) cppfront is still learning 'inspect' "
            "- only simple 'is' alternatives are currently supported");
        return;
      }
    }
    ...
    if (is_expression) {
      if (!found_wildcard) {
        errors.emplace_back(n.position(),
                            "an inspect expression must have an `is _` "
                            "match-anything wildcard alternative");
        return;
      }
    } else {
      printer.print_cpp2("}", n.close_brace);
  ...
  auto emit(iteration_statement_node const &n) -> void {
    ...
    if (*n.identifier == "while") {
      assert(n.condition && n.statements && !n.range && !n.body);
    ...
    //
      ...
      assert(!n.condition && !n.statements && n.range && n.parameter && n.body);
      ...
      //        the workaround to avoid their bugs for now is to emit a { }
      //        block around the Cpp1 range-for and make the scope variable a
      //        normal local
      ...
      //  If there's a next-expression, smuggle it in via a nested
      //  do/while(false) loop (nested "continue" will work, but "break" won't
      //  until we do extra work to implement that using a flag and implementing
      //  "break" as "_for_break = true; continue;")
      if (n.next_expression) {
        printer.print_cpp2(" { do ", n.position());
      }
    ...
    if (iteration_statements.back().stmt->label &&
      ...
      errors.emplace_back(
          ...
          name +
              ": a named loop must have its name used (did you forget 'break " +
              name + ";' or 'continue " + name + "';?)");
    }
  ...
  //-----------------------------------------------------------------------
  //
  auto check_return_expression_lifetime(expression_node const &n) -> bool {
    ...
    if (tok && is_forward_return) {
      //  Ensure we're not returning a local or an in/move parameter
      auto is_parameter_name =
          current_functions.back().decl->has_parameter_named(*tok);
      if (is_parameter_name &&
        ...
        errors.emplace_back(n.position(),
                            "a 'forward' return type cannot return an 'in', "
                            "'copy', or 'move' parameter");
        return false;
      } else if (!is_parameter_name && sema.get_declaration_of(*tok) &&
      ...
      //  Deduced forward returns are okay because decltype(auto) can deduce a
      //  value
      else if (!function_returns.back().is_deduced &&
               (is_literal(tok->type()) ||
                n.expr->is_result_a_temporary_variable())) {
        errors.emplace_back(
            n.position(),
            "a 'forward specific_type' return type must return an 'inout', "
            "'out', or 'forward' parameter; it cannot return a complex "
            "expression -- for example: for a function that takes a stream "
            "object 'output`, instead of 'return output << data;' write "
  ...
  auto emit(return_statement_node const &n) -> void {
    ...
    in_single_unqualified_id_return.push_back(
        n.expression && n.expression->is_unqualified_id());
    ...
    if (n.expression) {
      ...
      auto is_deduced_return =
          !function_returns.empty() && function_returns.back().is_deduced;
      ...
      if (function_returns.empty() ||
        ...
        errors.emplace_back(n.position(),
                            "return statement with expression must be in a "
                            "function with a single anonymous return value");
        return;
      }
    }

    else if (!function_returns.empty() &&
             function_returns.back().param_list == &single_anon) {
      errors.emplace_back(n.position(),
                          "return statement must have an expression in a "
                          "function with a single anonymous return value");
    }
    ...
    //
    else if (!function_returns.empty() && function_returns.back().param_list) {
      ...
      for (bool first = true; auto &param : parameters) {
        ...
        emit(*param->declaration->identifier,
             cpp2::unchecked_narrow<int>(parameters.size()));
        printer.emit_to_string();
      }
  ...
  auto emit(jump_statement_node const &n) -> void {
    ...
    if (n.label) {
      auto iter_stmt = std::find_if(iteration_statements.begin(),
                                    iteration_statements.end(), [&](auto &s) {
                                      assert(s.stmt);
                                      return s.stmt->label &&
                                             std::string_view{*s.stmt->label} ==
                                                 std::string_view{*n.label};
                                    });
      if (iter_stmt == iteration_statements.end()) {
        errors.emplace_back(
            n.position(),
            "a named " + n.keyword->to_string() +
                " must use the name of an enclosing local loop label");
        return;
      }
      iter_stmt->used = true;
      assert((*iter_stmt).stmt->label);
      printer.print_cpp2("goto " + to_upper_and_underbar(*n.keyword) + "_" +
                             (*iter_stmt).stmt->label->to_string() + ";",
                         n.position());
    } else {
      emit(*n.keyword);
      printer.print_cpp2(";", n.position());
  ...
  //-----------------------------------------------------------------------
  //
  auto emit(using_statement_node const &n) -> void {
    STACKINSTR
    if (n.export_) {
  ...
  //
  auto
  build_capture_lambda_intro_for(capture_group &captures, source_position pos,
    ...
    // If move from last use was used on the variable we need to rewrite the str
    // to add std::move to earlier use of the variable. That will save us from
    // capturing one variable two times (one with copy and one with std::move).
    for (auto rit = captures.members.rbegin(); rit != captures.members.rend();
         ++rit) {
      auto is_same_str_suppressed_move =
          [s = rit->str_suppressed_move](auto &cap) {
            return cap.str_suppressed_move == s;
          };

      auto rit2 = std::find_if(rit + 1, captures.members.rend(),
                               is_same_str_suppressed_move);
      while (rit2 != captures.members.rend()) {
        rit2->str = rit->str;
        rit2 = std::find_if(rit2 + 1, captures.members.rend(),
                            is_same_str_suppressed_move);
      }
    }
  ...
  auto emit(primary_expression_node const &n) -> void {
    ...
    if (n.expr.index() == primary_expression_node::declaration) {
      ...
      if (decl->is_function()) {
        auto lambda_intro =
            build_capture_lambda_intro_for(decl->captures, n.position());

        // Handle an anonymous generic function with explicit type list
        if (decl->template_parameters) {
          print_to_string(&lambda_intro, *decl->template_parameters, false,
                          true);
        }
      ...
      //  Else an anonymous object as 'typeid { initializer }'
        ...
        if (!decl->initializer) {
          errors.emplace_back(decl->position(),
                              "an anonymous object declaration must have '=' "
                              "and an initializer");
          return;
        }
  ...
  //  Not yet implemented. TODO: finalize deducing pointer types from parameter
  //  lists
  auto is_pointer_declaration(parameter_declaration_list_node const *, int, int)
      -> bool {
    return false;
  }

  auto is_pointer_declaration(declaration_node const *decl_node, int deref_cnt,
                              int addr_cnt) -> bool {
    if (!decl_node) {
      return false;
    }
  ...
  auto is_pointer_declaration(function_type_node const *fun_node, int deref_cnt,
                              int addr_cnt) -> bool {
    if (!fun_node) {
      return false;
    }
        ...
        [&]<typename T>(T const &type) {
          if constexpr (std::is_same_v<T, std::monostate>) {
            return false;
                                   ...
                                   T, function_type_node::single_type_id>) {
  ...
  auto is_pointer_declaration(type_id_node const *type_id_node, int deref_cnt,
                              int addr_cnt) -> bool {
    if (!type_id_node) {
      return false;
    }
    ...
    if (type_id_node->dereference_of) {
      return is_pointer_declaration(type_id_node->dereference_of,
                                    deref_cnt + type_id_node->dereference_cnt,
                                    addr_cnt);
    } else if (type_id_node->address_of) {
      return is_pointer_declaration(type_id_node->address_of, deref_cnt,
                                    addr_cnt + 1);
    }

    auto pointer_declarators_cnt =
        std::count_if(std::cbegin(type_id_node->pc_qualifiers),
                      std::cend(type_id_node->pc_qualifiers),
                      [](auto *q) { return q->type() == lexeme::Multiply; });

    if (pointer_declarators_cnt == 0 &&
        type_id_node->suspicious_initialization) {
      return is_pointer_declaration(type_id_node->suspicious_initialization,
                                    deref_cnt, addr_cnt);
    }

    return (unchecked_narrow<int>(pointer_declarators_cnt) + addr_cnt -
            deref_cnt) > 0;
  }
  ...
  auto is_pointer_declaration(alias_node const *, int, int) -> bool {
    return false;
  }

  auto is_pointer_declaration(declaration_sym const *decl, int deref_cnt,
                              int addr_cnt) -> bool {
    if (!decl) {
      return false;
    }
  ...
  auto is_pointer_declaration(token const *t, int deref_cnt = 0,
                              int addr_cnt = 0) -> bool {
    if (!t) {
      return false;
    }
  ...
  auto source_order_name_lookup(std::string_view identifier)
    ...
    for (auto first = current_names.rbegin(), last = current_names.rend() - 1;
         first != last; ++first) {
      if (auto decl = get_if<declaration_node const *>(&*first);
          decl && *decl && (*decl)->has_name(identifier)) {
        return *decl;
      } else if (auto using_ = get_if<active_using_declaration>(&*first);
                 using_ && using_->identifier &&
  ...
  auto lookup_finds_type_scope_function(id_expression_node const &n) -> bool {
    if (!n.is_unqualified()) {
      return false;
    }
    ...
    if (!lookup || get_if<active_using_declaration>(&*lookup)) {
      return false;
    }

    auto decl = get<declaration_node const *>(*lookup);

    if (!decl->is_function() || !decl->has_name() || !decl->parent_is_type()) {
      return false;
    }
  ...
  auto lookup_finds_variable_with_placeholder_type_under_initialization(
      id_expression_node const &n) -> bool {
    if (!n.is_unqualified()) {
      return false;
    }
    ...
    auto decl = get<declaration_node const *>(*lookup);
    if (decl && decl->has_name(*id.identifier)) {
      if (!decl->is_object() && !decl->is_object_alias()) {
        return false;
      }

      if (decl->is_object()) {
        auto type = &**get_if<declaration_node::an_object>(&decl->type);
        return type->is_wildcard() && contains(current_declarations, decl);
      }
      auto const &type =
          (**get_if<declaration_node::an_alias>(&decl->type)).type_id;
      return (!type || type->is_wildcard()) &&
  ...
  auto emit(
      ...
      postfix_expression_node &n, bool for_lambda_capture = false) -> void {
    STACKINSTR
    if (!sema.check(n)) {
      return;
    }
    ...
    auto guard = finally([this] {
      last_postfix_expr_was_pointer =
          active_postfix_expressions.back().is_pointer;
    ...
    if (emitting_move_that_function && n.expr->get_token() &&
        *n.expr->get_token() == "that") {
      if (n.ops.empty()) {
        if (!already_moved_that_members.empty()) {
          errors.emplace_back(n.position(),
                              "attempting to move from whole 'that' object "
                              "after a 'that.member' was already moved from");
          return;
        }
        ...
        for (auto i = already_moved_that_members.begin();
             i != already_moved_that_members.end(); ++i) {
          if (!*i || **i == *member) {
            errors.emplace_back(n.position(),
                                "attempting to move twice from 'that." +
                                    member->to_string() + "'");
            return;
          }
        }
    ...
    //  Ensure that forwarding postfix-expressions start with a forwarded
    //  parameter name
    //
    assert(!current_args.empty());
    if (current_args.back().pass == passing_style::forward) {
      ...
      if (!(decl && decl->parameter &&
            decl->parameter->pass == passing_style::forward)) {
        errors.emplace_back(n.position(),
                            n.expr->get_token()->to_string() +
                                " is not a forwarding parameter name");
      }
    }
    ...
    if (n.expr->expr.index() == primary_expression_node::id_expression) {
      ...
      if (id->id.index() == id_expression_node::unqualified) {
        ...
        //        - we don't deduce pointer types from
        ...
        if (is_pointer_declaration(unqual->identifier)) {
          if (n.ops.empty()) {
            ...
            if (op->type() == lexeme::PlusPlus ||
              ...
              errors.emplace_back(op->position(),
                                  op->to_string() +
                                      " - pointer arithmetic is illegal - use "
                                      "std::span or gsl::span instead");
              violates_bounds_safety = true;
            } else if (op->type() == lexeme::Tilde) {
              errors.emplace_back(
                  op->position(),
                  op->to_string() +
                      " - pointer bitwise manipulation is illegal - use "
                      "std::bit_cast to convert to raw bytes first");
            }
          }
        }
      }
    ...
    if (n.cap_grp && !for_lambda_capture) {
      ...
      //  the first *cap_grp .str_suppressed_move that matches us (which is what
      //  the lambda introducer generator used to create a lambda capture)
      suppress_move_from_last_use = true;
      auto my_sym = print_to_string(n, true);
      suppress_move_from_last_use = false;

      auto found = std::find_if(
          n.cap_grp->members.cbegin(), n.cap_grp->members.cend(),
          [my_sym](auto &cap) { return cap.str_suppressed_move == my_sym; });

      assert(found != n.cap_grp->members.cend() &&
    ...
    for (auto i = n.ops.rbegin(); i != n.ops.rend(); ++i) {
      ...
      if (i->op->type() == lexeme::LeftParen) {
        ...
        assert(i->op);
        assert(i->op_close);
        auto local_args = text_chunks_with_parens_position{
            {}, i->op->position(), i->op_close->position()};
      ...
      //  it means that it should be handled by UFCS
          ...
          //  That happens when it finds that the function identifier being
          //  called is the name of a variable with deduced type and we are in
          //  its initializer (e.g., x := y.x();) So lower it to a member call
          //  instead, the only possible valid meaning.
          && !lookup_finds_variable_with_placeholder_type_under_initialization(
                 *i->id_expr)) {
        ...
        if (std::ssize(i->id_expr->template_arguments()) > 0) {
          ...
          if (i->id_expr->is_qualified()) {
            ...
            //  And split the unqualified id in the function name as two macro
            //  arguments
            auto &id = *get<id_expression_node::qualified>(i->id_expr->id);
            funcname =
                "(" + print_to_string(id, false) + "::)," +
                print_to_string(*cpp2::impl::assert_not_null(id.ids.back().id),
                                0, true, true);
          }
        ...
        if (current_declarations.back()->is_namespace() ||
        ...
        if (funcname.starts_with("cpp2::move(*this).")) {
          ...
          prefix.emplace_back(ufcs_string + "(" + funcname + ")(",
                              args.value().open_pos);
        }
        suffix.emplace_back(")", args.value().close_pos);
      ...
      //
        ...
        if (!last_was_prefixed && i != n.ops.rbegin()) {
        ...
        if (flag_safe_null_pointers && i->op->type() == lexeme::Multiply) {
          prefix.emplace_back("cpp2::impl::assert_not_null(",
                              i->op->position());
        }
        if (flag_safe_null_pointers && i->op->type() == lexeme::Multiply) {
          suffix.emplace_back(")", i->op->position());
      ...
      //
        ...
        if (flag_safe_subscripts && i->op->type() == lexeme::LeftBracket &&
          ...
          suffix.emplace_back(i->op_close->to_string(),
                              i->op_close->position());
        }
        ...
        if (i->id_expr) {
          ...
          auto print = print_to_string(
              *i->id_expr,
              ...
              i->op->type() == lexeme::Dot ||
                  i->op->type() == lexeme::DotDot // member access
          );
        ...
        if (flag_safe_subscripts && i->op->type() == lexeme::LeftBracket &&
          ...
          if (auto lit = i->expr_list->expressions.front().expr->get_literal();
              lit && lit->get_token()->type() == lexeme::DecimalLiteral) {
            prefix.emplace_back("CPP2_ASSERT_IN_BOUNDS_LITERAL(",
                                i->op->position());
          } else {
        ...
        //  If this is a range expression, suppress emitting the .../..=
        //  operator Otherwise emit the postfix operator here
        else if (!i->last_expr) {
          suffix.emplace_back(i->op->to_string(), i->op->position());
        }
      }
    ...
    //  even if this is a definite last use (only do that when an rvalue is
    //  okay)
  ...
  auto emit(is_as_expression_node const &n) -> void {
    ...
    assert(n.expr && n.expr->get_postfix_expression_node() &&
           n.expr->get_postfix_expression_node()->expr);
    {
      auto &p = n.expr->get_postfix_expression_node()->expr;
      if (auto t = p->get_token();
          t && is_literal(t->type()) && t->type() != lexeme::StringLiteral &&
          t->type() != lexeme::FloatLiteral &&
          !std::get<primary_expression_node::literal>(p->expr)
               ->has_user_defined_suffix() &&
    ...
    for (auto i = n.ops.rbegin(); i != n.ops.rend(); ++i) {
      ...
      if (i->type) {
        if (i->type->is_wildcard()) {
          ...
          if (*i->op != "is") {
            errors.emplace_back(n.position(),
                                "'as _' wildcard is not allowed, specify a "
                                "concrete target type instead");
          } else if (std::ssize(n.ops) > 1) {
            errors.emplace_back(
                n.position(),
                "an 'is _' wildcard may only be used on its own, not in a "
                "chain with other 'is'/'as' in the same subexpression");
          }
        } else {
          ...
          if (op_name == "as") {
            if (type_name == "void") {
              errors.emplace_back(
                  n.position(), "'expression as void' is not allowed - use '_ "
                                "= expression' instead to discard the result");
            }
    ...
    if (as_on_literal) {
      auto last_pos = prefix.rfind('>');
      assert(last_pos != prefix.npos);
  ...
  auto emit(binary_expression_node<Name, Term> const &n) -> void {
      ...
      //  to get single evaluation via the lambda capture
        ...
        for (auto const &term : n.terms) {
          ...
          if (flag_safe_comparisons) {
            ...
            case lexeme::NotEqualComparison:
              errors.emplace_back(
                  n.position(), "!= comparisons cannot appear in a comparison "
                                "chain (see https://wg21.link/p0893)");
              return;
              break;
            default:
              lambda_body += ",";
        ...
        if (found_lt + found_gt + found_eq != 1) {
          errors.emplace_back(
              n.position(), "a comparison chain must be all < and <=, all > "
                            "and >=, or all == (see https://wg21.link/p0893)");
          return;
        }

        printer.print_cpp2("[" + lambda_capture + "]{ return " + lambda_body +
                               "; }()",
                           n.position());
    ...
    //  Else if this is an assignment expression, don't add cpp2::move on the
    //  lhs even if this is a definite last use (only do that when an rvalue is
    //  okay)
    if (!n.terms.empty() &&
    ...
    if (!n.terms.empty() && n.terms.front().op->type() == lexeme::Assignment &&
        n.expr->get_postfix_expression_node() &&
        n.expr->get_postfix_expression_node()
            ->get_first_token_ignoring_this() &&
        *n.expr->get_postfix_expression_node()
                ->get_first_token_ignoring_this() == "_") {
    ...
    if (!n.terms.empty() && last_postfix_expr_was_pointer) {
      ...
      if (is_assignment_operator(n.terms.front().op->type()) && rhs_tok &&
          (*rhs_tok == "nullptr" || is_digit((rhs_tok->as_string_view())[0]))) {
        errors.emplace_back(
            n.terms.front().op->position(),
            n.terms.front().op->to_string() +
                " - pointer assignment from null or integer is illegal");
        violates_lifetime_safety = true;
      } else if (*n.terms.front().op == "+" || *n.terms.front().op == "+=" ||
                 *n.terms.front().op == "-" || *n.terms.front().op == "-=") {
        errors.emplace_back(n.terms.front().op->position(),
                            n.terms.front().op->to_string() +
                                " - pointer arithmetic is illegal - use "
                                "std::span or gsl::span instead");
        violates_bounds_safety = true;
    ...
    for (auto const &x : n.terms) {
      ...
      if (x.op->type() == lexeme::Assignment && in_definite_init) {
        ...
        //  Otherwise, just emit the general expression as usual
          ...
          auto suffix = std::string{};
          if (flag_safe_zero_division && (x.op->type() == lexeme::Slash ||
                                          x.op->type() == lexeme::SlashEq)) {
            assert(last_expr &&
                   "ICE: shouldn't get here without having captured a pointer "
                   "to the numerator expression");
            if (auto lit = x.expr->get_literal();
                lit && lit->get_token()->type() == lexeme::DecimalLiteral) {
              printer.print_cpp2("CPP2_ASSERT_NOT_ZERO_LITERAL(CPP2_TYPEOF(" +
                                     print_to_string(*last_expr) + "),",
                                 x.op->position());
            } else {
              printer.print_cpp2("CPP2_ASSERT_NOT_ZERO(CPP2_TYPEOF(" +
                                     print_to_string(*last_expr) + "),",
                                 x.op->position());
            }
            suffix = ")";
  ...
  //-----------------------------------------------------------------------
  //
  auto emit(expression_list_node const &n, bool parens_ok = true) -> void {
    STACKINSTR
    auto add_parens = should_add_expression_list_parens() &&
                      !n.inside_initializer && parens_ok;
    add_parens |= n.is_fold_expression() &&
                  !(n.inside_initializer &&
                    current_declarations.back()->initializer->position() !=
                        n.open_paren->position());
    ...
    for (auto const &x : n.expressions) {
      ...
      if (x.pass != passing_style::in && x.pass != passing_style::in_ref) {
        assert(x.pass == passing_style::out || x.pass == passing_style::move ||
      ...
      if (x.pass == passing_style::move || x.pass == passing_style::out) {
        printer.print_cpp2(")", n.position());
      }
    }
  ...
  auto emit(expression_statement_node const &n, bool can_have_semicolon,
    ...
    if (!emitted) {
      ...
      //  take over direct control of emitting it without needing to
      //  go through the whole grammar, and surround it with braces
      if (generating_return && n.expr->is_expression_list() &&
  ...
  // Consider moving these `stack` functions to `common.h` to enable more
  // general use.

  template <typename T>
  auto stack_value(T &var, std::type_identity_t<T> const &value) -> auto {
    return finally([&var, old = std::exchange(var, value)]() { var = old; });
  }

  template <typename T>
  auto stack_element(std::vector<T> &cont, std::type_identity_t<T> const &value)
      -> auto {
    cont.push_back(value);
    return finally([&] { cont.pop_back(); });
  }

  template <typename T> auto stack_size(std::vector<T> &cont) -> auto {
    return finally([&, size = cont.size()] { cont.resize(size); });
  }

  template <typename T>
  auto stack_size_if(std::vector<T> &cont, bool cond)
  ...
  //-----------------------------------------------------------------------
  //
  auto emit(statement_node const &n, bool can_have_semicolon = true,
            source_position function_body_start = {},
            bool function_void_ret = false,
            function_prolog const &function_prolog = {},
            std::vector<std::string> const &function_epilog = {}) -> void {
    ...
    auto emit_parameters = !n.emitted && n.parameters;
    ...
    try_emit<statement_node::expression>(
        n.statement, can_have_semicolon, function_body_start, function_void_ret,
        function_prolog, function_epilog, n.emitted);

    //  Otherwise, skip this one if it was already handled earlier (i.e., a
    ...
    //  the original source's personal whitespace formatting style as much as we
    //  can
  ...
  //-----------------------------------------------------------------------
  //
  auto emit(parameter_declaration_node const &n, bool is_returns = false,
            bool is_template_parameter = false, bool is_statement = false)
      -> void {
    STACKINSTR
    if (!sema.check(n)) {
      return;
    ...
    //  Can't declare functions as parameters -- only pointers to functions
    //  which are objects
    ...
    if (n.mod == parameter_declaration_node::modifier::implicit) {
      ...
      if (n.pass != passing_style::out ||
        ...
        errors.emplace_back(n.position(),
                            "only an 'out this' parameter of an 'operator=' "
                            "function may be declared implicit");
      }
    }
    ...
    auto emit_initializer = [&] {
      ...
      auto emit_in_phase_0 = is_param_to_namespace_scope_type &&
      ...
      if (!is_returns && n.declaration->initializer &&
          (n.is_in_function_scope() // implies we're in phase 1 or 2
           || emit_in_phase_0 || emit_in_phase_1)) {
        auto expr =
            n.declaration->initializer->get_if<expression_statement_node>();
        assert(expr->expr);
    ...
    if (n.declaration->has_name("that")) {
      ...
      assert(n.pass == passing_style::in || n.pass == passing_style::move);
      auto pass = std::string{" const&"};
      if (n.pass == passing_style::move || emitting_move_that_function) {
        pass = "&&";
      }
      ...
      if (current_functions.back()
              .decl->get_parent()
              ->get_type_scope_declarations(declaration_node::objects)
              .empty()) {
        maybe_unused = "[[maybe_unused]] ";
      }

      printer.print_cpp2(maybe_unused + print_to_string(*type_name) + pass +
                             " that",
                         n.position());
      return;
    }
    ...
    auto emit_template_name = [&]() {
      ...
      if (identifier == "_") {
        printer.print_cpp2(
            unnamed_type_param_name(n.ordinal,
                                    n.declaration->identifier->get_token()),
            identifier_pos);
      } else {
    ...
    auto const &type_id =
        *std::get<declaration_node::an_object>(n.declaration->type);
    ...
    assert(n.declaration->identifier);
    auto param_type =
        print_to_string(type_id, source_position{},
    ...
    //  template argument
    auto has_template_parameter_type_named = [](declaration_node const &decl,
                                                std::string_view name) -> bool {
      if (decl.template_parameters) {
        for (auto &tparam : decl.template_parameters->parameters) {
          assert(tparam && tparam->name());
    ...
    auto is_dependent_parameter_type =
        has_template_parameter_type_named(*current_declarations.back(),
                                          param_type) ||
        (current_declarations.back()->parent_is_type() &&
         current_declarations.back()->has_name("operator=") &&
         has_template_parameter_type_named(
             *current_declarations.back()->get_parent(), param_type));
    ...
    if (!is_returns && !n.declaration->is_variadic && !type_id.is_wildcard() &&
        !is_dependent_parameter_type && !type_id.is_pointer_qualified()) {
      switch (n.pass) {
        break;
    ...
    if (type_id.is_pointer_qualified() && n.pass == passing_style::in) {
      printer.print_cpp2(param_type, n.position());
    } else if (type_id.is_wildcard() || is_dependent_parameter_type ||
               n.declaration->is_variadic) {
      auto name = std::string{"auto"};
      if (is_dependent_parameter_type) {
        name = param_type;
      } else if (n.declaration->is_variadic && !type_id.is_wildcard()) {
      ...
      case passing_style::inout:
        ...
        //  For generic out parameters, we take a pointer to anything with
        //  paramater named "identifier_" and then generate the out<> as a stack
        //  local with the expected name "identifier"
        break;
      case passing_style::out:
        printer.print_cpp2(name, n.position());
    ...
    if (!is_returns && !type_id.is_wildcard() && !is_dependent_parameter_type &&
        !type_id.is_pointer_qualified() && !n.declaration->is_variadic) {
      switch (n.pass) {
        break;
      case passing_style::in:
        printer.print_cpp2(">", n.position());
    ...
    if (is_returns) {
      ...
      if (n.declaration->is_variadic) {
        if (n.direction() == passing_style::out) {
          errors.emplace_back(n.declaration->position(),
  ...
  //-----------------------------------------------------------------------
  //
  auto emit(parameter_declaration_list_node const &n, bool is_returns = false,
            bool is_template_parameter = false,
            bool generating_postfix_inc_dec = false) -> void {
    STACKINSTR
    ...
    //  So we don't get cute about text-aligning the first parameter when it's
    //  on a new line
    printer.disable_indent_heuristic_for_next_text();
  ...
  auto emit(
    ...
    if (n.group) {
      ...
      if (name == "cpp2_default" || name == "bounds_safety" ||
          name == "null_safety" || name == "type_safety" || name == "testing") {
        name.insert(0, "cpp2::");
      }
    }

    //  "Unevaluated" is for static analysis only, and are never evaluated, so
    //  just skip them (The only requirement for an Unevaluated condition is
    //  that it parses; and even that's easy to relax if we ever want to allow
    //  arbitrary tokens in an Unevaluated condition)
    ...
    if (*n.kind == "post") {
      auto lambda_intro =
          build_capture_lambda_intro_for(n.captures, n.position(), true);
      printer.print_cpp2("cpp2_finally_presuccess.add(" + lambda_intro + "{",
                         n.position());
    }
    ...
    printer.print_cpp2("if (", n.position());
    for (auto const &flag : n.flags) {
      printer.print_cpp2(separator + print_to_string(*flag), n.position());
      separator = " && ";
    }
    printer.print_cpp2(separator + name + ".is_active()", n.position());
    printer.print_cpp2(" && !(" + print_to_string(*n.condition) + ") ) " +
  ...
  //-----------------------------------------------------------------------
  //
  auto emit(function_type_node const &n, bool is_main = false,
            bool is_ctor_or_dtor = false, std::string const &suffix1 = {},
    ...
    auto handle_single_anon_return_type = [&] {
      auto is_type_scope_function_with_in_this =
          n.my_decl && n.my_decl->parent_is_type() &&
    ...
    if (is_main && n.parameters->parameters.size() > 0) {
      printer.print_cpp2("(int const argc_, char** argv_)",
    ...
    //  For an anonymous function, the emitted lambda is 'constexpr' or
    //  'mutable'
    if (n.my_decl && !n.my_decl->has_name()) {
      if (n.my_decl->is_constexpr) {
        //  The current design path we're trying out is for all '==' functions
        //  to be emitted as Cpp1 'constexpr', including anonymous functions.
        //  For anonymous functions that have captures, the intent is that '=='
        //  implies "the result always the same (depends only on the
        //  arguments)." Specifically, the result doesn't depend on the captured
        //  state, so the captured state should be const. But until we want to
        //  take a dependency on post-C++20 constexpr relaxation to make more
        //  code work with 'constexpr' even when not invoked in constexpr
        //  contexts, we will emit it as const/whitespace instead for now.
        //
        //  printer.print_cpp2( " constexpr", n.position() );
        //      // consider enabling when P2242, P2280, and similar papers are
        //      widely implemented
    ...
    if (n.is_move() || n.is_swap() || n.is_destructor() ||
        (n.my_decl && generating_move_from == n.my_decl)) {
    ...
    if (n.is_assignment() ||
        (n.my_decl && generating_assignment_from == n.my_decl)) {
      assert(n.returns.index() == function_type_node::empty &&
  ...
  //-----------------------------------------------------------------------
  //
  auto is_name_declared_in_current_type_scope(std::string_view s) -> bool {
    if (!s.empty()) {
      //  Navigate to the enclosing type, if there is one...
      for (auto parent = current_declarations.rbegin();
           parent != current_declarations.rend(); ++parent) {
        if (*parent && (*parent)->is_namespace()) {
          break;
        }
        //  ... and here it is, so...
  ...
  //-----------------------------------------------------------------------
  //
  auto get_enclosing_type_name() -> token const * {
    //  Navigate to the enclosing type, if there is one...
    for (auto parent = current_declarations.rbegin();
         parent != current_declarations.rend(); ++parent) {
      if (*parent && (*parent)->is_namespace()) {
  ...
  //-----------------------------------------------------------------------
  //
  auto get_enclosing_function_name() -> token const * {
  ...
  //
  auto type_qualification_if_any_for(declaration_node const &n) -> std::string {
    auto ret = std::string{};

    if (printer.get_phase() == printer.phase2_func_defs && n.parent_is_type()
  ...
  //
  auto emit_special_member_function(declaration_node const &n,
                                    std::string const &prefix) -> void {
    STACKINSTR
    assert(n.is_function());
    auto &func = std::get<declaration_node::a_function>(n.type);
    assert(func);

    auto is_assignment = generating_assignment_from == &n ||
                         (*func->parameters)[0]->pass == passing_style::inout;

    if (func->parameters->ssize() > 1 &&
        (*func->parameters)[1]->has_name("that")) {
      emitting_that_function = true;
      if ((*func->parameters)[1]->pass == passing_style::move ||
    ...
    if (printer.get_phase() == printer.phase2_func_defs) {
      auto canonize_object_name =
          [&](declaration_node const *obj) -> std::string {
        assert(obj->has_name());
      ...
      auto error_msg =
          "an operator= body must start with a series of 'member = value;' "
          "initialization statements for each of the type-scope objects in the "
          "same order they are declared, or the member must have a default "
          "initializer (in type '" +
          n.parent_declaration->name()->to_string() + "')";

      //  If this constructor's type has data members, handle their
      //  initialization
      //      - objects is the list of this type's declarations
      //      - statements is the list of this constructor's statements
      auto objects =
          n.parent_declaration->get_type_scope_declarations(n.objects);
      ...
      while (object != objects.end()) {
        ...
        auto is_object_before_base =
            n.get_decl_if_type_scope_object_name_before_a_base_type(
                *(*object)->name());
        ...
        //  Otherwise, use a default... for a non-copy/move that's the member
        //  initializer (for which we don't need to emit anything special
        //  because it will get used), and for a copy/move function we default
        //  to "= that.same_member" (or, if this is a base type, to assigning
        //  from the lowered base subobject)
        if (!found_explicit_init) {
          if (emitting_that_function && (*object)->has_name("this")) {
            ...
            initializer = "static_cast<" + object_name + pass + ">(that)";
        ...
        if (!found_explicit_init && !found_default_init) {
          errors.emplace_back(stmt_pos,
                              "in operator=, expected '" + object_name +
                                  " = ...' initialization statement (because "
                                  "type scope object '" +
                                  object_name +
                                  "' does not have a default initializer)");
          errors.emplace_back((*object)->position(),
                              "see declaration for '" + object_name + "' here");
          errors.emplace_back(stmt_pos, error_msg);
          return;
        }

        assert(found_explicit_init || found_default_init);
        ...
        if (is_assignment) {
          ...
          //  Else just use infix assignment
            ...
            current_functions.back().prolog.statements.push_back(
                object_name + " = " + initializer + ";");
          }
        }
        //  (b) ... if this isn't assignment, only need to emit it if it was
        //          explicit, or is a base type or 'that' initializer
        else if (found_explicit_init || is_object_before_base ||
                 ((*object)->has_name("this") && !initializer.empty()) ||
                 emitting_that_function) {
          if (is_object_before_base) {
            assert(is_object_before_base->name());
            object_name =
                print_to_string(
                    *is_object_before_base->parent_declaration->name()) +
                "_" + (*object)->name()->to_string() + "_as_base";
          }
          ...
          if (!out_inits_with_commas.empty()) {
            current_functions.back().prolog.mem_inits.push_back(
                separator + object_name + "{(" + out_inits_with_commas +
                initializer + " )}");
          } else {
            if (initializer == "{}") {
              initializer = "";
            ...
            current_functions.back().prolog.mem_inits.push_back(
                separator + object_name + "{ " + initializer + " }");
          }
          separator = ", ";
        }
      ...
      if (object != objects.end()) {
        errors.emplace_back(
            (*object)->position(),
            canonize_object_name(*object) +
                " was not initialized - did you forget to write a default "
                "initializer, or assign to it in the operator= body?");
        errors.emplace_back((*object)->position(),
                            "see declaration for '" +
                                canonize_object_name(*object) + "' here");
        errors.emplace_back((*object)->position(), error_msg);
        return;
      }
    ...
    //  For a constructor, print the type name instead of the operator= function
    //  name
    assert(n.parent_is_type());
    if (!is_assignment) {
      printer.print_cpp2(prefix, n.position());
      printer.print_cpp2(type_qualification_if_any_for(n), n.position());
      printer.print_cpp2(print_to_string(*n.parent_declaration->name()),
                         n.position());
    ...
    //  For an assignment operator, similar to emitting an ordinary function
      ...
      printer.print_cpp2(prefix, n.position());
      printer.print_cpp2("auto " + type_qualification_if_any_for(n) +
                             print_to_string(*n.name()),
                         n.position());
      emit(*func);
    }
  }
  ...
  //-----------------------------------------------------------------------
  //
  auto emit(declaration_node const &n, std::string const &capture_intro = {})
      -> void {
    STACKINSTR
      ...
      while (parent && parent->is_type()) {
        if (parent->requires_clause_expression) {
          parent_template_parameters =
              "requires( " +
              print_to_string(*parent->requires_clause_expression) + " )\n" +
              parent_template_parameters;
        }
        if (parent->template_parameters) {
          parent_template_parameters =
              "template " +
              print_to_string(*parent->template_parameters, false, true) + " " +
              parent_template_parameters;
        }
        parent = parent->parent_declaration;
      }
    ...
    auto const emit_requires_clause = [&]() {
      if (n.requires_clause_expression ||
        ...
        if (printer.get_phase() == printer.phase1_type_defs_func_decls) {
          // Workaround GCC 10 not supporting requires in forward declarations
          // in some cases. See commit 5a0d77f8e297902c0b9712c5aafb6208cfa4c139.
        ...
        if (!function_requires_conditions.empty()) {
          ...
          for (auto it = std::cbegin(function_requires_conditions) + 1;
               it != std::cend(function_requires_conditions); ++it) {
    ...
    if (printer.get_phase() == printer.phase0_type_decls && !n.is_namespace() &&
        !n.is_type()) {
      return;
    }
    ...
    if (n.is_alias()) {
      ...
      if ((!n.parent_is_function() &&
           printer.get_phase() == printer.phase1_type_defs_func_decls) ||
          (n.parent_is_type() && n.is_object_alias() &&
           printer.get_phase() == printer.phase2_func_defs) ||
          (n.parent_is_function() &&
           printer.get_phase() == printer.phase2_func_defs)) {
        assert(a->is_type_alias() || a->is_namespace_alias() ||
               a->is_object_alias());
        ...
        if (a->is_object_alias() && n.parent_is_type() &&
            printer.get_phase() == printer.phase2_func_defs) {
        ...
        //  Handle type aliases
        if (a->is_type_alias()) {
          printer.print_cpp2("using " + print_to_string(*n.identifier) + " = " +
                                 print_to_string(*std::get<alias_node::a_type>(
                                     a->initializer)) +
                                 ";\n",
                             n.position());
        }
        ...
        //  Handle namespace aliases
          ...
          printer.print_cpp2(
              "namespace " + print_to_string(*n.identifier) + " = " +
                  print_to_string(
                      *std::get<alias_node::a_namespace>(a->initializer)) +
                  ";\n",
              n.position());
        }
        ...
        //      - at type scope, it's also inline constexpr but see note (*)
        //      below
          ...
          auto print_initializer_to_string =
              [&](expression_node const &n) -> std::string {
            //  If this expression is just a single expression-list, we can
            //  take over direct control of emitting it without needing to
            //  go through the whole grammar, and surround it with braces
            if (n.is_expression_list()) {
              return "{ " + print_to_string(*n.get_expression_list(), false) +
                     " }";
            }
            //  Otherwise, just emit the general expression as usual
            else {
              return "{ " + print_to_string(n) + " }";
            }
          };

          //  (*) If this is at type scope, Cpp1 requires an out-of-line
          //  declaration dance for some cases to work - see
          //  https://stackoverflow.com/questions/11928089/
          if (n.parent_is_type()) {
            assert(n.parent_declaration->name());

            if (printer.get_phase() == printer.phase1_type_defs_func_decls) {
              printer.print_cpp2("static const " + type + " " +
                                     print_to_string(*n.identifier) + ";\n",
                                 n.position());
            } else if (printer.get_phase() == printer.phase2_func_defs) {
              //  The following logic is not yet complete, so give a diagnostic
              //  for now
              if (n.parent_declaration->parent_is_type()) {
                errors.emplace_back(
                    n.position(),
                    "(temporary alpha limitation) an object alias cannot yet "
                    "appear inside a nested type");
                return;
              }

              printer.print_cpp2(
                  "inline CPP2_CONSTEXPR " + type + " " +
                      type_qualification_if_any_for(n) +
                      print_to_string(*n.identifier) +
                      print_initializer_to_string(
                          *std::get<alias_node::an_object>(a->initializer)) +
                      ";\n",
                  n.position());
            }
          }
          //  Otherwise, at function and namespace scope we can just define
            ...
            printer.print_cpp2(
                type + " " + intro + " " + print_to_string(*n.identifier) +
                    print_initializer_to_string(
                        *std::get<alias_node::an_object>(a->initializer)) +
                    ";\n",
                n.position());
    ...
    if (n.is_function() && n.has_name()) { // reset the 'that' flags
      emitting_that_function = false;
      emitting_move_that_function = false;
      already_moved_that_members = {};
    }

    auto is_main = !n.parent_declaration && n.has_name("main");
    ...
    if (printer.get_phase() == printer.phase1_type_defs_func_decls &&
      ...
      if (func->returns.index() == function_type_node::list) {
        ...
        if (std::ssize(r->parameters) == 1) {
          printer.print_extra(
              "\nusing " + func_name + "_ret = " +
              print_to_string(
                  *r->parameters[0]->declaration->get_object_type()) +
              ";");
        }
    ...
    if (n.is_type() &&
      ...
      auto &compound_stmt =
          std::get<statement_node::compound>(n.initializer->statement);
      ...
      for (auto &stmt : compound_stmt->statements) {
        if (stmt->is_declaration()) {
          ...
          auto emit_as_base =
              decl->get_decl_if_type_scope_object_name_before_a_base_type(
                  *decl->name());

          if (emit_as_base) {
            printer.print_extra(
                "\nstruct " +
                print_to_string(*decl->parent_declaration->name()) + "_" +
                decl->name()->to_string() + "_as_base { " +
    ...
    if (n.is_export() && n.parent_is_namespace()) {
    ...
    //  is one, or default to private for data and public for functions
      ...
      if (!n.is_default_access() && !n.is_export()) {
        assert(is_in_type);
        printer.print_cpp2(to_string(n.access) + ": ", n.position());
      } else if (is_in_type) {
        if (n.is_object()) {
    ...
    //  Print a line directive before every function definition, excluding
    //  lambdas. This is needed to enable debugging with lldb.
    if (printer.get_phase() == printer.phase2_func_defs && n.is_function() &&
        n.has_name() && n.initializer) {
      printer.print_extra("");
    }
    ...
    if (n.template_parameters &&
        (printer.get_phase() < printer.phase2_func_defs || n.is_object() ||
         (n.is_function() &&
          n.has_name()     // only if it is not unnamed function aka lambda
          && n.initializer // only if the function has a definition (is not
                           // abstract)
          && printer.get_phase() == printer.phase2_func_defs)) &&
        (!n.is_concept() ||
         printer.get_phase() == printer.phase1_type_defs_func_decls)) {
    ...
    if (n.is_type()) {
      ...
      auto saved_for_body =
          std::vector<std::pair<std::string, source_position>>{};
      ...
      for (auto &stmt : compound_stmt->statements) {
        assert(stmt);
        if (!stmt->is_declaration() && !stmt->is_using()) {
          //  We will already have emitted an error for this in sema.check
          return;
        }

        //  If it's a using statement, save it up if we haven't started the body
        //  yet
        ...
        if (decl->is_alias() &&
          ...
          if (started_body) {
            ...
            saved_for_body.emplace_back(print_to_string(*decl),
                                        decl->position());
          }
          continue;
        }
        ...
        //  and any data members declared before them that we push into private
        //  bases
        assert(decl->name());
        auto emit_as_base =
            decl->get_decl_if_type_scope_object_name_before_a_base_type(
        ...
        if (emit_as_base) {
          ...
          if (decl->has_name("this")) {
            if (printer.get_phase() == printer.phase1_type_defs_func_decls) {
              printer.print_cpp2(separator + " public " +
                                     print_to_string(*decl->get_object_type()),
      ...
      if (printer.get_phase() == printer.phase1_type_defs_func_decls) {
        ...
        auto indent = static_cast<size_t>(
            std::clamp(compound_stmt->body_indent, n.position().colno,
        ...
        if (n.member_function_generation) {
          ...
          if (!found_that_constructor) {
            printer.print_extra(prefix + id + "(" + id +
                                " const&) = delete; /* No 'that' constructor, "
                                "suppress copy */");
            printer.print_extra(prefix + "auto operator=(" + id +
                                " const&) -> void = delete;");
          }
    ...
    if (n.is_namespace()) {
      ...
      if (auto tok = n.identifier->get_token(); tok && *tok != "_") {
        emit(*n.identifier);
      }
      ...
      for (auto &stmt : compound_stmt->statements) {
        ...
        if (stmt->is_declaration()) {
          ...
          errors.emplace_back(
              stmt->position(),
              "a namespace scope must contain only declarations or 'using' "
              "statements, not other code");
          return;
        }
      }
    ...
    //  Function
             ...
             (printer.get_phase() < printer.phase2_func_defs ||
              n.initializer // only emit definition if the function has one (is
                            // not abstract)
              || n.is_defaultable_function())) {
      auto is_streaming_operator = [](std::string_view sv) {
        return sv == "operator<<" || sv == "operator>>";
      };

      auto is_binary_arithmetic_operator = [](std::string_view sv) {
        return sv == "operator+" || sv == "operator-" || sv == "operator*" ||
      ...
      auto emit_as_friend =
          ...
          (is_streaming_operator(n.name()->as_string_view()) ||
           (!n.is_function_with_this() &&
            is_binary_arithmetic_operator(n.name()->as_string_view())));
      ...
      current_functions.push(&n, func.get(),
                             n.find_parent_declared_value_set_functions());
      auto guard0 = finally([&] { current_functions.pop(); });

      auto guard1 = stack_size(current_names);

      //  If this is at expression scope, we can't emit "[[nodiscard]] auto
      //  name" so print the provided intro instead, which will be a Cpp1
      //  lambda-introducer
      if (capture_intro != "") {
      ...
      //  Else start introducing a normal function
        ...
        if (!n.has_initializer() && n.is_defaultable_function()) {
          suffix2 += " = default";
        }

        //  If there's a 'this' parameter, handle it here (the parameter
        //  emission will skip it) because Cpp1 syntax requires its information
        //  to be spread around the declaration syntax
        ...
        if (!func->parameters->parameters.empty() &&
          ...
          case passing_style::inout:
            ...
            break;
          case passing_style::out:; // constructor is handled below
            break;
          case passing_style::move:
            suffix1 += " &&";
          ...
          default:
            errors.emplace_back(n.position(),
                                "ICE: invalid parameter passing style, should "
                                "have been rejected",
                                true);
          }
          ...
          if (printer.get_phase() != printer.phase2_func_defs) {
            ...
            case parameter_declaration_node::modifier::final_:
              ...
              break;
            default:
              if (func->is_constructor() && !func->is_constructor_with_that() &&
        ...
        //  Else if there isn't a 'this' parameter, but this function is in a
        //  type scope, it's a Cpp1 non-member function so we need to say so (on
        ...
        //  If there's a return type, it's [[nodiscard]] implicitly and all the
        //  time
        //  -- for now there's no opt-out, wait and see whether we actually need
        //  one
        if (func->has_non_void_return_type() && !func->is_assignment() &&
            !func->is_compound_assignment() &&
            !func->is_increment_or_decrement() &&
            (printer.get_phase() == printer.phase1_type_defs_func_decls ||
             n.has_initializer() // so we're printing it in phase 2
             ) &&
            (!emit_as_friend // can't have an attribute on a friend
                             // declaration-not-definition
             || printer.get_phase() != printer.phase1_type_defs_func_decls) &&
            !(n.name() && is_streaming_operator(n.name()->as_string_view()))) {
        ...
        if (n.is_constructor() || n.is_assignment()) {
          assert(!is_main && suffix2.empty() &&
                 "ICE: an operator= shouldn't have been able to generate a "
                 "suffix (or be main)");

          emit_special_member_function(n, prefix);
          ...
          if (!n.parent_is_polymorphic() && func->parameters->ssize() == 2 &&
              generating_assignment_from != &n) {
            assert(!current_functions.empty());
            ...
            if (
                ...
                (&n == current_functions.back()
                           .declared_value_set_functions.out_this_in_that &&
                 !current_functions.back()
                      .declared_value_set_functions.inout_this_in_that) ||
                //  A2) This is '(out   this, move that)'
                //      and no  '(inout this, move that)' was written by the
                //      user
                //  (*) and no  '(inout this,      that)' was written by the
                //  user (*)
                //
                //  (*) This third test is to tie-break M2 and A2 in favor of
                //  M2. Both M2 and A2
                //      can generate a missing '(inout this, move that)', and if
                //      we have both options then we should prefer to use M2
                //      (generate move assignment from copy assignment) rather
                //      than A2 (generate move assignment from move
                //      construction) as M2 is a better fit (move assignment is
                //      more like copy assignment than like move construction,
                //      because assignments are designed structurally to set the
                //      value of an existing 'this' object)
                (&n == current_functions.back()
                           .declared_value_set_functions.out_this_move_that &&
                 !current_functions.back()
                      .declared_value_set_functions.inout_this_move_that &&
                 !current_functions.back()
                      .declared_value_set_functions.inout_this_in_that) ||
                //  A3) This is '(out   this, something-other-than-that)'
                (n.is_constructor() && !n.is_constructor_with_that() &&
                 !contains(current_functions.back()
                               .declared_value_set_functions.assignments_from,
                           n.nth_parameter_type_name(2)))) {
            ...
            if (generating_move_from != &n) {
              ...
              if (
                  ...
                  //      and no  '(out   this, move that)' was written by the
                  //      user
                  (&n == current_functions.back()
                             .declared_value_set_functions.out_this_in_that &&
                   !current_functions.back()
        ...
        //  For a destructor, we need to translate
          ...
          //  Print the ~-prefixed type name instead of the operator= function
          //  name
          assert(n.parent_is_type() && n.parent_declaration->name());
          printer.print_cpp2(prefix + type_qualification_if_any_for(n) + "~" +
        ...
        //  don't emit abstract virtual functions in phase 2
          ...
          emit(*n.name());
          emit(*func, is_main, false, suffix1,
               generating_postfix_inc_dec_from != nullptr);
          printer.print_cpp2(suffix2, n.position());

          //  If this is ++ or --, also generate a Cpp1 postfix version of the
          //  operator (as long as we don't know for sure this isn't a copyable
          //  type)
          if (func->is_increment_or_decrement()) {
            if (generating_postfix_inc_dec_from) {
              assert(generating_postfix_inc_dec_from == &n);
            } else if (!n.parent_declaration ||
      ...
      //  Else emit the definition
        ...
        if (func->returns.index() == function_type_node::list) {
          ...
          function_returns.emplace_back(
              ...
              std::get<function_type_node::id>(func->returns)
                  .type->is_wildcard());
        } else {
        ...
        if (func->returns.index() == function_type_node::list) {
          ...
          for (auto &param : r->parameters) {
            ...
            if (decl.initializer) {
              ...
              auto &expr = std::get<statement_node::expression>(
                  decl.initializer->statement);
              assert(expr);
        ...
        if (generating_postfix_inc_dec_from) {
          ...
          if (!n.parent_declaration || !n.parent_declaration->is_type()) {
            param1 = n.first_parameter_name();
          }

          printer.print_cpp2(" { auto ret = " + param1 + "; ++" + param1 +
                                 "; return ret; }",
                             n.position());
        }
        //  Else just emit the normal function body
        else {
          emit(*n.initializer, true, func->position(),
      ...
      //  If this is ++ or --, emit the Cpp1 postfix version via a recursive
      //  call
    ...
    //  Object with optional initializer
      ...
      if (printer.get_phase() != printer.phase2_func_defs &&
          n.parent_is_namespace() && !type->is_concept()) {
      ...
      if (n.has_name("_")) {
        ...
        //  Minimize diff noise by using ordinals for generated names, not
        //  line/col info
        ...
        printer.print_cpp2("auto_" + next_unique_id(),
                           n.identifier->position());
      ...
      if (n.initializer) {
        ...
        if (type->is_concept() || type->is_function_typeid()) {
          printer.print_cpp2(" = ", n.position());
        } else {
          printer.print_cpp2(" {", n.position());
        }
  ...
  auto print_errors() -> void {
    ...
    bool print_fallback_errors =
        true; // true until we find a non-fallback message
    ...
    if (violates_lifetime_safety) {
      std::cerr << "  ==> program violates lifetime safety guarantee - see "
                   "previous errors\n";
    }
    if (violates_bounds_safety) {
      std::cerr << "  ==> program violates bounds safety guarantee - see "
                   "previous errors\n";
    }
    if (violates_initialization_safety) {
      std::cerr << "  ==> program violates initialization safety guarantee - "
                   "see previous errors\n";
    }
  }
  ...
  //
  auto has_cpp1() const -> bool { return source.has_cpp1(); }

  //-----------------------------------------------------------------------
  //  has_cpp2: pass through
  //
  auto has_cpp2() const -> bool { return source.has_cpp2(); }
};
